    ...
    if !block_exists {
        ...
        for (idx, &ins_raw) in bc_raw.iter().enumerate().skip(block_start_idx as usize) {
            ...
            match disasm(ins_raw)? {
                ...
                Op::JMP(_, jump) | Op::ISNEXT(_, jump) => {
                    graph.add_node(
                        block_start_idx,
                        Block::from_ins_vec(
...
mod tests {
    ...
    fn it_works() -> Result<(), DecompileError> {
        let bc: Vec<u32> = vec![
            822, 50398009, 50463545, 197671, 33686338, 196622, 2147812440, 822, 50398009, 50463545,
            263207, 33686338, 1078, 67175481, 67437625, 394535, 33686594, 262158, 2147550552,
            459815, 1334, 83952953, 84215097, 525863, 33686850, 327694, 2147550808, 460071, 1590,
            100730425, 100992569, 591655, 33687106, 393230, 2147551064, 460327, 1846, 117507897,
            118097721, 722983, 198930, 789031, 2870, 184617785, 184683321, 855079, 33688386,
            134940710, 33687362, 458766, 2147551320, 460583, 2091, 459782, 2147617112, 918535,
            2148534616, 985398, 152045881, 1116711, 264972, 2147552088, 1182503, 168495654, 2857,
            3113, 17041730, 1247542, 264722, 16910658, 67883, 133452, 2159937880, 1311751,
            2149386584, 1378614, 264722, 461586, 396306, 33818946, 589838, 2148272472, 985398,
            152439097, 1509941, 985910, 186125113, 720910, 2147552344, 2868, 169413437, 16910658,
            67883, 133452, 985398, 152045881, 264722, 1706791, 168495654, 2857, 3113, 17041730,
            132172, 2157906264, 1771830, 33622338, 589839, 2156923480, 1837366, 50399554, 589838,
            2148928344, 1903414, 1969191, 33688386, 720910, 2148600664, 1903414, 2034742, 396562,
            202181670, 33688386, 720910, 2148141912, 985910, 185994041, 2100277, 658700,
            2147552600, 2166055, 203558205, 16911170, 68395, 133964, 2859, 3113, 3371, 67112489,
            920577, 2150174552, 2150109013, 2297639, 790546, 2363687, 252776230, 4150, 270864441,
            270929977, 987410, 2560551, 286396710, 33689666, 1052946, 271061049, 2691623, 463655,
            33820738, 1051922, 265477, 2148405336, 4150, 270864441, 270929977, 987410, 2757159,
            286396710, 33689666, 1052946, 271061049, 2691623, 463655, 33820738, 1050642,
            2147946328, 3338, 2147815512, 462087, 2147553368, 2147618648, 201329686, 2144669528,
            2058, 2148339544, 2821942, 528402, 33689410, 985106, 67593, 2147553112, 67625, 2887478,
            252186424, 984076, 2147553368, 2950183, 2149257048, 69419, 3018806, 987446, 288297273,
            67244098, 2147816280, 3085366, 336794680, 267269, 2147554392, 134955, 33755973,
            2147029842, 983055, 2147553368, 2148208728, 987190, 269488185, 3150119, 4649, 4905,
            17043522, 1249334, 266514, 16912450, 69675, 135244, 1380150, 266258, 463122, 397842,
            33820482, 983054, 2148274008, 986934, 253103929, 3215413, 987446, 286789945, 1114126,
            2147553880, 4404, 270078269, 16912194, 69419, 134988, 986934, 252710713, 266258,
            1708327, 269553702, 4393, 4649, 17043266, 132172, 2148206936, 985398, 152045881,
            3279399, 2857, 3113, 17041730, 1247542, 264722, 16910658, 67883, 133452, 65611,
        ];
